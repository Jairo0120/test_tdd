...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_partial_update_recipe(self):
        ...
        recipe = sample_recipe(user=self.user)
        recipe.tags.add(sample_tag(user=self.user))
        new_tag = sample_tag(user=self.user, name='Otra vaina')

        payload = {
            'title': 'Titulo cambiad',
            'tags': [new_tag.id]
        }
        url = detail_url(recipe.id)
...
class RecipeImageUploadTests(TestCase):
    ...
    def test_upload_image_bad_request(self):
        ...
        url = image_upload_url(self.recipe.id)
        res = self.client.post(url, {'image': 'someshit'}, format='multipart')

        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)

    def test_filter_recipes_by_tags(self):
        """Test returning recipes with specific tags"""
        recipe1 = sample_recipe(user=self.user, title='Receta 1')
        recipe2 = sample_recipe(user=self.user, title='Receta 2')
        tag1 = sample_tag(user=self.user, name='Tag 1')
        tag2 = sample_tag(user=self.user, name='Tag 2')
